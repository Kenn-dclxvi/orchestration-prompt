...
## 証明道具箱方式への成熟
採用後の運用設計は、固定された Plan / Audit / Review の起動判断から、希望成果物の安全証明に必要な証明ツールを選び、必要に応じて独立実行枠へ分離する証明道具箱方式へ成熟した。これは現行正本 `docs/orchestration-process.md` に反映済みの運用ルールであり、この文書では rationale として扱う。

成熟後の中心判断は、「Plan SA を使うか / Audit SA を使うか / Review SA を使うか」ではない。「希望成果物の安全証明に必要な証明ツールは何か」「それらを同じ独立実行枠にまとめてよいか」「分離する必要があるなら、その理由は並列稼働・情報制御・バイアス制御のどれか」である。

この成熟は、v2 の Toolbox / Contract / Quality Gate を廃止するものではない。Toolbox は安全証明に使う証明ツールの道具箱として再解釈され、Contract は実行側に渡す最終権威として明確化された。構成判断は実行契約を生成せず、Scope / Done / Test / Permission を補完しない。

軽量化の意味も、工程や情報分離を削ることから、安全証明に不要な過剰コストを減らすことへ整理された。判断ログがない省略は軽量化ではなく手抜きであり、情報制御やバイアス制御を緩める根拠にはならない。
...
証明道具箱方式への成熟後は、Toolbox を希望成果物の安全証明に使う証明ツールの一覧として読む。証明ツールは、観点、制約、入力契約、出力契約、禁止事項、確認項目、証拠要求のまとまりであり、独立実行枠そのものではない。独立実行枠は、選択された証明ツールを載せて動く分離コンテキストである。
