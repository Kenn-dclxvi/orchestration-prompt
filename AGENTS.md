...
- 親の初動は、固定SA選択ではなく、希望成果物の安全証明に必要な証明ツールと独立実行枠を決める構成判断から始める。
- 証明道具箱・証明ツール・独立実行枠・構成判断・判断ログの定義は `docs/orchestration-process.md` に従う。
- 独立実行枠の起動は `docs/orchestration-process.md` の構成判断と SA利用ケースに該当する場合だけ行う。
...
## 指示書草案
- ユーザーが指示書作成を明示依頼した場合、または実装・変更の意図がありながら作業単位化に必要な境界項目を推定なしで確定できない場合、親は構成判断により計画整理ツールを選択し、`prompts/plan.md` を載せた独立実行枠を起動する。起動条件・非対象・受け渡し契約・安全不変条件は `docs/orchestration-process.md` のケースEと Parent→Plan 契約に従う。
...
## SA起動と分離
- 親は最小証明セットから始め、Risk / Ambiguity / Impact / Proof Gap に応じて証明ツールを追加する。
- 独立実行枠への分離は、並列稼働・情報制御・バイアス制御のいずれかに意味がある場合だけ行い、単に便利そう、速そうという理由では分離しない。
- 親は、判断ログとして残す項目を `docs/orchestration-process.md` の正本項目に従って記録する。判断ログがない省略は軽量化として扱わない。
- 軽量化は安全性削減ではなく証明コスト削減であり、情報分離、バイアス制御、停止条件、除外操作を緩和しない。
- 実行契約は実行側に渡す最終権威であり、構成判断は実行許可・Scope・Done・Test・Permission を生成しない。
- 各SA起動時は、対応するロールプロンプト(`docs/orchestration-process.md` のサブエージェント起動の対応表)と禁止操作を起動メッセージに明示する。
- 監査SAへは Audit Contract と必要な関連仕様のみ渡す。実装経緯・背景・親の事前評価は渡さない。
- レビューSAへは「指示書・差分・テスト結果・必要な関連仕様・レビュー観点・監査SAの停止指摘0件という起動ゲート事実」のみ渡す。実装経緯・事前評価・監査結果本文・他SAの指摘内容・判断理由・個別評価を渡さない。
